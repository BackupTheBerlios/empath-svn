...
import unittest
from validate.value import ValidateValueMapping

class TestValidateValueMapping_And(unittest.TestCase): #{{{
    def setUp(self): #{{{
        pass
    # End def #}}}
    ...
    def testSimpleMappingType(self): #{{{
        ...
        v = ValidateValueMapping(*dict(hello=1, world=2).items())
    ...
    def testNonMappingValue(self): #{{{
        ...
        v = ValidateValueMapping(dict(hello=1))
        val = 1
